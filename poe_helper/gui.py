...
class Gui(ttk.Frame):
    ...
    def create_label_styles(self):
        """Create text color style for labels."""
        style = ttk.Style()
    ...
    def create_listing_labels(self, resp):
        ...
        for counter, item in enumerate(resp):
            self.price_title_label = ttk.Label(self.price_label_frame, 
                text=f'{counter+1}:'
            )
            self.price_result_label = ttk.Label(self.price_label_frame, 
                text=item.format_price()
            )
            self.price_result_label.grid(column=1, row=counter+10, sticky="e", 
                padx=10, pady=2
            )
    ...
    def display_listings(self):
        ...
        try:
            ...
            self.listing_label = ttk.Label(self.price_label_frame, 
                text="Listings:"
            )
